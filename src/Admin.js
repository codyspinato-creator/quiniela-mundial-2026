...
export default function Admin({ onBack }) {
  ...
  const setKO = (rondaId, idx, field, val) => {
    setResultados(prev => {
      ...
      // Propagate winners to next round automatically
      const updatedKO = { ...prev.knockout, [rondaId]: arr };
      try {
        return { ...prev, knockout: buildBracket(prev.scores, updatedKO) };
      } catch(e) {
        return { ...prev, knockout: updatedKO };
      }
    });
  };
    ...
    <div style={{ minHeight: "100vh", background: B.bg, color: B.text, fontFamily: "Georgia,serif" }}>
      ...
      <div style={{ maxWidth: 620, margin: "0 auto", padding: "14px 12px 56px" }}>
        ...
        {!loading && adminTab === "grupos" && (
          <>
            <div style={{ background: `${B.admin}15`, border: `1px solid ${B.admin}40`, borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 11, color: B.muted }}>
              📝 Ingresa los resultados <strong style={{ color: B.text }}>oficiales</strong> de cada partido. Al guardar los grupos, los clasificados se asignan automáticamente a la fase eliminatoria.
            </div>
            ...
            <div style={{ background: "#ffffff", border: "1px solid #e0e0e8", borderRadius: 10, padding: "12px 14px", textAlign: "center" }}>
              <div style={{ fontSize: 11, color: B.muted, marginBottom: 8 }}>
                Cuando hayas ingresado todos los resultados de grupos, calcula automáticamente los cruces de eliminatorias:
              </div>
              {autoMsg && <div style={{ fontSize: 11, color: B.admin, fontWeight: "bold", marginBottom: 8 }}>{autoMsg}</div>}
              <button onClick={autoPopulateBracket} style={{ padding: "9px 20px", borderRadius: 9, border: "none", background: `linear-gradient(135deg,${B.admin},#6d28d9)`, color: "#ffffff", fontWeight: "bold", fontSize: 13, cursor: "pointer" }}>
                ⚡ Auto-calcular Eliminatorias
              </button>
            </div>
          </>
        )}
        ...
        {!loading && adminTab === "knockout" && (
          <>
            <div style={{ background: `${B.admin}15`, border: `1px solid ${B.admin}40`, borderRadius: 10, padding: "10px 14px", marginBottom: 10, fontSize: 11, color: B.muted }}>
              📝 Los equipos se llenan automáticamente desde los grupos. Ingresa los marcadores y selecciona el ganador de cada partido.
            </div>
            ...
            <div style={{ background: "#f5f0ff", border: `1px solid ${B.admin}40`, borderRadius: 10, padding: "10px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ flex: 1, fontSize: 11, color: B.admin }}>
                {autoMsg || "¿Los equipos no aparecen? Recalcula los cruces desde grupos."}
              </div>
              <button onClick={autoPopulateBracket} style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: B.admin, color: "#ffffff", fontWeight: "bold", fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" }}>
                ⚡ Recalcular
              </button>
            </div>
              ...
              return (
                <div style={{ display: ronda.partidos >= 4 ? "grid" : "block", gridTemplateColumns: ronda.partidos >= 4 ? "1fr 1fr" : "1fr", gap: 8 }}>
                  {matches.map((match, i) => {
                    ...
                    return (
                      <div key={i} style={{ background: B.card, border: `1px solid ${match.ganador ? B.admin + "80" : "#e0e0e8"}`, borderRadius: 12, padding: "10px 12px", marginBottom: ronda.partidos < 4 ? 8 : 0, boxShadow: match.ganador ? `0 2px 8px ${B.admin}20` : "0 1px 4px rgba(0,0,0,0.05)" }}>
                        ...
                        <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
                          {/* Local - editable si no hay equipo auto, read-only si lo hay */}
                          {hasTeams && match.local ? (
                            <div style={{ flex: 1, fontSize: 11, textAlign: "right", fontWeight: match.ganador === match.local ? "bold" : "normal", color: match.ganador === match.local ? B.admin : "#111", background: "#f0f4ff", border: `1px solid ${match.ganador === match.local ? B.admin : "#c8d8ff"}`, borderRadius: 6, padding: "6px 8px" }}>
                              {match.local}
                            </div>
                          ) : (
                            <input value={match.local} onChange={e => setKO(rondaActiva, i, "local", e.target.value)} placeholder="Local..." style={{ flex: 1, background: "#f8f8fc", border: "1px solid #e0e0e8", borderRadius: 6, padding: "6px 8px", color: "#111", fontSize: 11, outline: "none" }} />
                          )}
                          <input type="number" min="0" max="20" value={match.localGoles} onChange={e => setKO(rondaActiva, i, "localGoles", e.target.value)}
                            style={{ width: 36, height: 32, textAlign: "center", background: "#f0f0f0", border: `2px solid ${hasScore ? B.admin : "#e0e0e8"}`, borderRadius: 6, color: hasScore ? B.admin : "#bbb", fontSize: 14, fontWeight: "bold", outline: "none" }} />
                        </div>
                        <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: empate ? 8 : 0 }}>
                          {hasTeams && match.visita ? (
                            <div style={{ flex: 1, fontSize: 11, fontWeight: match.ganador === match.visita ? "bold" : "normal", color: match.ganador === match.visita ? B.admin : "#111", background: "#f0f4ff", border: `1px solid ${match.ganador === match.visita ? B.admin : "#c8d8ff"}`, borderRadius: 6, padding: "6px 8px" }}>
                              {match.visita}
                            </div>
                          ) : (
                            <input value={match.visita} onChange={e => setKO(rondaActiva, i, "visita", e.target.value)} placeholder="Visitante..." style={{ flex: 1, background: "#f8f8fc", border: "1px solid #e0e0e8", borderRadius: 6, padding: "6px 8px", color: "#111", fontSize: 11, outline: "none" }} />
                          )}
                          <input type="number" min="0" max="20" value={match.visitaGoles} onChange={e => setKO(rondaActiva, i, "visitaGoles", e.target.value)}
                            style={{ width: 36, height: 32, textAlign: "center", background: "#f0f0f0", border: `2px solid ${hasScore ? B.admin : "#e0e0e8"}`, borderRadius: 6, color: hasScore ? "#111" : "#bbb", fontSize: 14, fontWeight: "bold", outline: "none" }} />
                        </div>
                          ...
                          <div style={{ marginTop: 8, padding: "6px 8px", background: "#f8f8fc", borderRadius: 8, border: "1px solid #e8e8f0" }}>
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                              {[match.local, match.visita].filter(Boolean).map(eq => (
                                <button key={eq} onClick={() => { setKO(rondaActiva, i, "penaltis", true); setKO(rondaActiva, i, "penaltisGanador", eq); setKO(rondaActiva, i, "ganador", eq); }}
                                  style={{ flex: 1, padding: "5px 8px", borderRadius: 7, border: `1px solid ${match.ganador === eq ? B.admin : "#e0e0e8"}`, background: match.ganador === eq ? "#f5f0ff" : "#ffffff", color: match.ganador === eq ? B.admin : "#888", fontSize: 11, cursor: "pointer", fontWeight: match.ganador === eq ? "bold" : "normal" }}>
                          ...
                          <div style={{ marginTop: 8 }}>
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                              {[{ eq: match.local, wins: gl > gv }, { eq: match.visita, wins: gv > gl }].filter(x => x.eq).map(({ eq, wins }) => (
                                <button key={eq} onClick={() => setKO(rondaActiva, i, "ganador", eq)}
                                  style={{ flex: 1, padding: "5px 8px", borderRadius: 7, border: `1px solid ${match.ganador === eq ? B.admin : "#e0e0e8"}`, background: match.ganador === eq ? "#f5f0ff" : "#f8f8fc", color: match.ganador === eq ? B.admin : "#888", fontSize: 10, cursor: "pointer", fontWeight: match.ganador === eq ? "bold" : "normal" }}>
        ...
        {!loading && adminTab === "especiales" && (
          <div>
            <div style={{ background: `${B.admin}15`, border: `1px solid ${B.admin}40`, borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 11, color: B.muted }}>
              📝 Define los resultados oficiales. Se asignan 5 puntos por acierto.
            </div>
            {[
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                  {SELECCIONES.map(s => (
                    <button key={s} onClick={() => setResultados(prev => ({ ...prev, [field]: s === prev[field] ? "" : s }))}
                      style={{ padding: "5px 10px", borderRadius: 18, fontSize: 11, border: `1px solid ${resultados[field] === s ? color : "#e0e0e8"}`, background: resultados[field] === s ? color + "15" : "#f8f8fc", color: resultados[field] === s ? color : "#555", cursor: "pointer", fontWeight: resultados[field] === s ? "bold" : "normal" }}>
                      {s}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {GOLEADORES.map(g => (
                  <button key={g.nombre} onClick={() => setResultados(prev => ({ ...prev, goleador: g.nombre === prev.goleador ? "" : g.nombre }))}
                    style={{ padding: "7px 10px", borderRadius: 8, textAlign: "left", border: `1px solid ${resultados.goleador === g.nombre ? "#9b5de5" : "#e0e0e8"}`, background: resultados.goleador === g.nombre ? "#f5f0ff" : "#f8f8fc", color: resultados.goleador === g.nombre ? "#9b5de5" : "#555", cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                    <span>{g.sel}</span>
                    <div style={{ flex: 1 }}><div style={{ fontSize: 11, fontWeight: resultados.goleador === g.nombre ? "bold" : "normal" }}>{g.nombre}</div>{g.club && <div style={{ fontSize: 9, color: "#aaa" }}>{g.club}</div>}</div>
        ...
        {!loading && adminTab === "codigo" && (
          <div>
            <div style={{ background:`${B.admin}15`, border:`1px solid ${B.admin}40`, borderRadius:10, padding:"12px 14px", marginBottom:18, fontSize:11, color:B.muted }}>
              🔑 Define el código que los participantes necesitan para entrar.
            </div>
            <div style={{ background:"#ffffff", border:"1px solid #e0e0e8", borderRadius:14, padding:20, marginBottom:14 }}>
              ...
              <div style={{ fontSize:11, color:B.muted, marginBottom:16 }}>Si lo cambias, las personas con el código antiguo no podrán volver a entrar (sus quinielas se conservan).</div>
              <div style={{ position:"relative", marginBottom:12 }}>
                <input type="text" value={codigoAcceso} onChange={e => setCodigoAcceso(e.target.value)} placeholder="Escribe el código..."
              ...
              <button onClick={saveCodigo} style={{ width:"100%", padding:12, borderRadius:9, border:"none", background:codigoSaved ? "#00c853" : `linear-gradient(135deg,${B.admin},#6d28d9)`, color:"#ffffff", fontWeight:"bold", fontSize:14, cursor:"pointer" }}>
                {codigoSaved ? "✓ Código guardado" : "Guardar código"}
              </button>
            </div>
            {codigoAcceso && (
              <div style={{ background:"#ffffff", border:`2px dashed ${B.admin}50`, borderRadius:12, padding:16, textAlign:"center" }}>
                <div style={{ fontSize:10, color:B.muted, letterSpacing:3, textTransform:"uppercase", marginBottom:8 }}>Comparte este código</div>
                <div style={{ fontSize:32, fontWeight:"900", color:B.admin, letterSpacing:8, fontFamily:"monospace" }}>{codigoAcceso}</div>
              </div>
            )}
          </div>
        )}
        ...
        {!loading && adminTab === "ranking" && (
          <div>
            <div style={{ background: `${B.admin}15`, border: `1px solid ${B.admin}40`, borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 11, color: B.muted }}>
              🏅 Puntuación en tiempo real. Guarda los resultados para actualizar.
            </div>
            {confirmDelete && (
              <div style={{ background:"#fff8f0", border:"1px solid #f77f00", borderRadius:8, padding:"8px 12px", marginBottom:10, fontSize:11, color:"#7a4000", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <span>⚠️ Clic en "¿Seguro?" para confirmar la eliminación</span>
                <button onClick={()=>setConfirmDelete(null)} style={{background:"none",border:"none",color:"#7a4000",cursor:"pointer",fontSize:13}}>✕</button>
              </div>
            )}
              ...
              {ranking.map((p, rank) => (
                <div key={p.id} style={{ display: "grid", gridTemplateColumns: "32px 1fr 180px 36px 36px 36px 52px 52px", gap: 4, padding: "10px 12px", borderTop: "1px solid #eeeeef", background: rank === 0 ? "#eff4ff" : "transparent", alignItems: "center" }}>
                  ...
                  <div style={{ fontSize: 10, color: "#555", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {p.email || <span style={{ color: "#ccc", fontStyle: "italic" }}>—</span>}
                  </div>
                  ...
                  <div style={{ textAlign: "center" }}>
                    <button onClick={() => deleteQuiniela(p.id, p.nombre)} disabled={deletingId === p.id}
                      style={{ padding: "4px 8px", borderRadius: 6, border: "none", fontSize: 11, background: confirmDelete === p.id ? "#e63946" : "#fff0f0", color: confirmDelete === p.id ? "#ffffff" : "#e63946", cursor: "pointer", fontWeight: confirmDelete === p.id ? "bold" : "normal", whiteSpace: "nowrap" }}>
            ...
            <div style={{ background: "#ffffff", border: "1px solid #e0e0e8", borderRadius: 12, padding: 14 }}>
              ...
              <div style={{ marginTop: 10, padding: "8px 10px", background: "#f8f8fc", borderRadius: 8, fontSize: 10, color: B.muted }}>
                📊 Máximo: <strong style={{color:B.text}}>360</strong> + <strong style={{color:B.text}}>160</strong> + <strong style={{color:B.text}}>20</strong> = <strong style={{color:B.primary}}>540 pts</strong>
              </div>
            </div>
          </div>
        )}
